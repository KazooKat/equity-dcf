...
with st.sidebar:
    ...
    wacc_pct = st.slider("Discount rate / WACC (%)", 6.0, 15.0, 9.0, 0.25)
    growth_pct = st.slider("FCF growth, year 1 (%)", -10.0, 25.0, 8.0, 0.5,
                           help="Defaults are a neutral starting point — the historical FCF CAGR is shown below for reference.")
    terminal_pct = st.slider("Terminal growth (%)", 0.0, 4.0, 2.5, 0.25,
                             help="Long-run growth into perpetuity; usually at or below nominal GDP growth (~2-3%).")

    st.header("Market price")
    st.caption("Auto-fetched when available; override if the quote is stale or missing.")
    price_override = st.number_input("Price override ($, 0 = use auto quote)", min_value=0.0, value=0.0, step=1.0)
...
try:
    ...
    st.error(f"**{ticker}** isn't in the SEC's company list. Check the ticker — foreign or OTC "
             "listings without EDGAR filings won't work.")
    st.stop()
except ValueError as e:
    st.error(f"EDGAR has filings for **{ticker}** but not the annual facts this model needs: {e}. "
             "Banks and insurers often report under different concepts than the standard "
             "revenue/CFO/capex set this DCF is built on.")
    st.stop()
...
if result:
    ...
    t4.metric("Enterprise value", f"${result.enterprise_value / 1e9:,.0f}B")
...
with tab_val:
    if result:
        ...
        proj_tbl = result.projection.copy()
        proj_tbl["growth"] = (proj_tbl["growth"] * 100).round(2)
        table_view(proj_tbl.round(0), "Table view — projection")

        st.subheader("Sensitivity — implied price across WACC × terminal growth")
        st.caption("Neutral gray = today's market price; blue = implied value above market, red = below.")
